...
void main() {
    ...
    if(block_type > 2.5) {  // Water
    ...
    } else {  // Other blocks (including iron blocks and glass)
        block_color = texture2D(tex, texcoord);
        ...
        } else if(block_type > 0.5 && block_type < 1.5) {  // Iron block (assuming block_type == 1.0)
            // Calculate reflection for iron block
            vec3 reflect_vec = reflect(normalize(fragposition), surface_normal);
            vec3 norm_reflect_vec = normalize(reflect_vec);
            vec3 reflect_color = mix(low_sky_color, hi_sky_color, sqrt(clamp(dot(norm_reflect_vec, up_vec), 0.0001, 1.0)));
            reflect_color = xyz_to_rgb(reflect_color);

            #if defined CLOUD_REFLECTION && (V_CLOUDS != 0 && !defined UNKNOWN_DIM) && !defined NETHER
                reflect_color = get_cloud(normalize((gbufferModelViewInverse * vec4(reflect_vec * far, 1.0)).xyz), reflect_color, 0.0, dither, worldposition.xyz, int(CLOUD_STEPS_AVG * 0.5), umbral, cloud_color, dark_cloud_color);
            #endif

            // Fresnel for metallic reflection
            float metal_fresnel = pow(1.0 - max(normal_dot_eye, 0.0), 4.0) * 0.8 + 0.2;
            // Apply metallic tint (iron-like color)
            vec3 metal_tint = vec3(0.9, 0.9, 0.95); // Slightly bluish-gray for iron
            block_color.rgb = mix(block_color.rgb * metal_tint, reflect_color, metal_fresnel * 0.5);
        }
    }
